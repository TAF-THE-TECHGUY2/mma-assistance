...
/** A case joined with its outpatient detail record, as rendered in the register. */
interface OutpatientRow {
  case: MedicalCase;
  detail: OutpatientDetail | null;
}

/** Mutable fields the operations team edits on an outpatient record. */
...
const EMPTY_FORM: OutpatientForm = {
  file_date: '',
  file_number: '',
  consult_date: '',
  followup_date: '',
...
function detailToForm(detail: OutpatientDetail | null): OutpatientForm {
  if (!detail) return { ...EMPTY_FORM };
  return {
    file_date: toDateInput(detail.file_date),
    file_number: detail.file_number ?? '',
    consult_date: toDateInput(detail.consult_date),
    followup_date: toDateInput(detail.followup_date),
    ongoing_treatment: Boolean(detail.ongoing_treatment),
...
export default function OutpatientRegister() {
  ...
  const [ongoingFilter, setOngoingFilter] = useState<string>('all'); // all | ongoing | completed

  // Edit modal state.
  const [editing, setEditing] = useState<OutpatientRow | null>(null);
  const [form, setForm] = useState<OutpatientForm>(EMPTY_FORM);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const built = await Promise.all(
        cases.map(async (c) => {
          if (c.outpatient_detail) {
            return { case: c, detail: c.outpatient_detail } satisfies OutpatientRow;
          }
          try {
            const detail = await getOutpatientDetail(c.id);
            return { case: c, detail } satisfies OutpatientRow;
          } catch {
            return { case: c, detail: null } satisfies OutpatientRow;
          }
        })
      );
      ...
      const message =
        ...
        'Failed to load the outpatient register.';
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (ongoingFilter === 'ongoing' && !r.detail?.ongoing_treatment) return false;
      if (ongoingFilter === 'completed' && r.detail?.ongoing_treatment) return false;
      return true;
    });
  }, [rows, statusFilter, ongoingFilter]);
  ...
  const handlePrint = useCallback(() => {
    ...
    const columns: PrintColumn<OutpatientRow>[] = [
      ...
      { header: 'Consult Date', value: (r) => d(r.detail?.consult_date) },
      { header: 'FU Date', value: (r) => d(r.detail?.followup_date) },
      { header: 'Ongoing Treatment', value: (r) => (r.detail?.ongoing_treatment ? 'Yes' : 'No') },
      ...
      { header: 'Date Submitted', value: (r) => d(r.detail?.submission_date) },
      { header: 'Date Pastel', value: (r) => d(r.detail?.date_pastel) },
    ];
    printRegister({ title: 'OUT PATIENT MANAGEMENT REGISTER', columns, rows: filtered, orientation: 'landscape', minRows: 15 });
  }, [filtered]);
  ...
  const handleExport = useCallback(async () => {
    ...
    setError(null);
    try {
      await downloadRegisterExcel('outpatient');
    } catch {
  ...
  const openEditor = useCallback((row: OutpatientRow) => {
    setEditing(row);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const payload = {
        file_date: nullable(form.file_date),
        file_number: nullable(form.file_number),
        consult_date: nullable(form.consult_date),
        followup_date: nullable(form.followup_date),
      ...
      const updated = await updateOutpatientDetail(editing.case.id, payload);
      setRows((prev) =>
      ...
      const message =
        (err as { response?: { data?: { message?: string } } })?.response?.data?.message ||
        'Failed to save outpatient details.';
  ...
  const columns = useMemo<Column<OutpatientRow>[]>(
    () => [
      {
        key: 'case_number',
        header: 'Case #',
      ...
      },
      {
        ...
        header: 'Consult',
        sortable: true,
        accessor: (r) => r.detail?.consult_date ?? '',
        render: (r) => fmtDate(r.detail?.consult_date),
      },
      {
        key: 'followup_date',
        header: 'Follow-up',
        sortable: true,
        accessor: (r) => r.detail?.followup_date ?? '',
        render: (r) => fmtDate(r.detail?.followup_date),
      },
      {
        key: 'ongoing_treatment',
        header: 'Ongoing',
        sortable: true,
        accessor: (r) => (r.detail?.ongoing_treatment ? 1 : 0),
        render: (r) => <YesNo value={Boolean(r.detail?.ongoing_treatment)} />,
      },
      {
        key: 'mr_requested',
        header: 'MR Req.',
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-slate-800">Outpatient Register</h1>
            <p className="text-sm text-slate-500">
              Track consults, follow-ups, and ongoing treatment for outpatient cases.
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center gap-2">
          <label htmlFor="op-status" className="text-sm font-medium text-slate-600">
            Status
          </label>
          <select
            id="op-status"
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="op-ongoing" className="text-sm font-medium text-slate-600">
            Treatment
          </label>
          <select
            id="op-ongoing"
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <DataTable<OutpatientRow>
          ...
          emptyMessage="No outpatient cases found."
        />
      </div>
...
interface EditModalProps {
  row: OutpatientRow;
  form: OutpatientForm;
  setForm: Dispatch<SetStateAction<OutpatientForm>>;
  saving: boolean;
...
function EditModal({ row, form, setForm, saving, saveError, onClose, onSave }: EditModalProps) {
  const set = <K extends keyof OutpatientForm>(key: K, value: OutpatientForm[K]) =>
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="w-full max-w-2xl overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-800">Edit Outpatient Details</h2>
        ...
        <div className="max-h-[70vh] overflow-y-auto px-6 py-5">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label="Consult date">
              <input
                ...
                onChange={(e) => set('consult_date', e.target.value)}
                className={inputCls}
              />
            </Field>
            ...
            <Field label="Case status">
              <input
                ...
                placeholder="e.g. awaiting follow-up"
                className={inputCls}
              />
            </Field>
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-6">
            <label className="flex items-center gap-2 text-sm text-slate-700">
              <input
                type="checkbox"
                checked={form.ongoing_treatment}
                onChange={(e) => set('ongoing_treatment', e.target.checked)}
